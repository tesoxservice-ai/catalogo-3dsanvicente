...
export default function HomePage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (showFavs) products = products.filter((p) => isFavorite(p.id));
    if (activeCategory !== "todos") products = products.filter((p) => p.category === activeCategory);
    return products;
  }, [activeCategory, showFavs, favorites]);
        ...
        <div className="fixed bottom-6 left-1/2 z-30" style={{ transform: "translateX(-50%)" }}>
          ...
          >
            <div className="relative">
              ...
              <span
                className="absolute -top-2 -right-2 w-4 h-4 rounded-full text-[9px] font-black flex items-center justify-center"
