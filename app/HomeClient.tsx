...
export default function HomeClient({ products }: { products: Product[] }) {
  ...
  const { cartCount, cartTotal, setCartOpen, favorites, isFavorite } = useCartFav();

  const filtered = useMemo(() => {
    let list = products;
    if (showFavs) list = list.filter((p) => isFavorite(p.id));
    if (activeCategory !== "todos") list = list.filter((p) => p.category === activeCategory);
    return list;
  }, [activeCategory, showFavs, favorites, products]);
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: "#FFFFFF", color: "#1A1A1A" }}>
      ...
      {/* HERO */}
      <section className="pt-14 pb-10 px-4 text-center" style={{ backgroundColor: "#FFFFFF", borderBottom: "1px solid #F3F4F6" }}>
        <h1 className="text-5xl sm:text-7xl font-black uppercase leading-none" style={{ color: "#1A1A1A", letterSpacing: "-0.03em" }}>
          San Vicente
        </h1>
        <p className="text-xs font-medium uppercase mt-3" style={{ color: "#9CA3AF", letterSpacing: "0.4em" }}>
          Impresión 3D Profesional
        </p>
      </section>

      {/* CATEGORÍAS */}
      <div style={{ backgroundColor: "#FFFFFF", borderBottom: "1px solid #E5E7EB", position: "sticky", top: 0, zIndex: 30 }}>
        <div className="max-w-screen-xl mx-auto px-4">
          <div className="flex items-center gap-1 overflow-x-auto" style={{ scrollbarWidth: "none" }}>
            ...
            <button
              ...
              className="cat-scroll whitespace-nowrap text-sm font-semibold transition-all duration-200 shrink-0 flex items-center gap-1.5"
              style={
                showFavs
                  ? { color: "#EF4444", borderBottom: "2px solid #EF4444", paddingBottom: "16px", paddingTop: "16px", paddingLeft: "12px", paddingRight: "12px", backgroundColor: "transparent" }
                  : { color: "#9CA3AF", borderBottom: "2px solid transparent", paddingBottom: "16px", paddingTop: "16px", paddingLeft: "12px", paddingRight: "12px", backgroundColor: "transparent" }
              }
            >
              <svg width="13" height="13" viewBox="0 0 24 24" fill={showFavs ? "#EF4444" : "none"} stroke={showFavs ? "#EF4444" : "#9CA3AF"} strokeWidth="2" strokeLinecap="round">
            ...
            {/* Categorías */}
            {CATEGORIES.map((cat) => {
              const isActive = cat.id === activeCategory && !showFavs;
              return (
                <button
                  key={cat.id}
                  ...
                  style={
                    isActive
                      ? { color: "#1A1A1A", borderBottom: "2px solid #1A1A1A", paddingBottom: "16px", paddingTop: "16px", paddingLeft: "12px", paddingRight: "12px", backgroundColor: "transparent" }
                      : { color: "#9CA3AF", borderBottom: "2px solid transparent", paddingBottom: "16px", paddingTop: "16px", paddingLeft: "12px", paddingRight: "12px", backgroundColor: "transparent" }
                  }
                >
                  {cat.label}
      ...
      {/* PRODUCTOS */}
      <main className="flex-1 w-full max-w-screen-xl mx-auto px-4 pt-8 pb-32" style={{ backgroundColor: "#F9FAFB" }}>
        <div className="flex items-center justify-between mb-5">
          <p className="text-sm" style={{ color: "#9CA3AF" }}>
            <span style={{ color: "#1A1A1A", fontWeight: 600 }}>{filtered.length}</span>{" "}
            producto{filtered.length !== 1 ? "s" : ""} · {showFavs ? "Favoritos" : activeLabel}
          </p>
        </div>
        ...
        {filtered.length > 0 ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4 rounded-2xl" style={{ border: "1px dashed #E5E7EB", backgroundColor: "#FFFFFF" }}>
            {showFavs ? (
              <>
                <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#E5E7EB" strokeWidth="1.5" strokeLinecap="round">
                  <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/>
                </svg>
                <p style={{ color: "#9CA3AF" }} className="text-sm">No tenés favoritos guardados</p>
              </>
            ) : (
              <>
      ...
      {cartCount > 0 && (
        <div className="fixed bottom-6 left-1/2 z-30" style={{ transform: "translateX(-50%)" }}>
          <button
            ...
            className="flex items-center gap-3 px-5 py-3.5 rounded-2xl shadow-xl transition-all duration-200 hover:scale-105 active:scale-95"
            style={{ backgroundColor: "#1A1A1A", color: "#FFFFFF", boxShadow: "0 8px 32px rgba(0,0,0,0.2)" }}
          >
            <div className="relative">
              ...
              <span className="absolute -top-2 -right-2 w-4 h-4 rounded-full text-[9px] font-black flex items-center justify-center" style={{ backgroundColor: "#25D366", color: "#FFFFFF" }}>
                {cartCount}
              </span>
            </div>
            <span className="text-sm font-bold">Ver pedido</span>
            <span className="text-sm font-black" style={{ color: "#9CA3AF" }}>·</span>
      ...
      <footer className="py-8 px-4" style={{ borderTop: "1px solid #E5E7EB", backgroundColor: "#FFFFFF" }}>
        <div className="max-w-screen-xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex flex-col items-center sm:items-start gap-0.5">
            <span className="font-black text-sm tracking-widest uppercase" style={{ color: "#1A1A1A" }}>San Vicente</span>
            <span className="text-[9px] tracking-widest uppercase" style={{ color: "#9CA3AF" }}>Impresión 3D Profesional</span>
          </div>
